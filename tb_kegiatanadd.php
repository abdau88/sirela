...
// Create page object
$tb_kegiatan_add = new tb_kegiatan_add();
...
var ftb_kegiatanadd = currentForm = new ew.Form("ftb_kegiatanadd", "add");

// Validate form
ftb_kegiatanadd.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php if ($tb_kegiatan_add->kode_kegiatan->Required) { ?>
		...
		<?php } ?>
		<?php if ($tb_kegiatan_add->nama_kegiatan->Required) { ?>
		...
		<?php } ?>
		<?php if ($tb_kegiatan_add->kode_kegiatan_sub1->Required) { ?>
...
ftb_kegiatanadd.Form_CustomValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
...
ftb_kegiatanadd.validateRequired = <?php echo json_encode(CLIENT_VALIDATE) ?>;

// Dynamic selection lists
ftb_kegiatanadd.lists["x_kode_kegiatan_sub1"] = <?php echo $tb_kegiatan_add->kode_kegiatan_sub1->Lookup->toClientList() ?>;
ftb_kegiatanadd.lists["x_kode_kegiatan_sub1"].options = <?php echo JsonEncode($tb_kegiatan_add->kode_kegiatan_sub1->lookupOptions()) ?>;
ftb_kegiatanadd.autoSuggests["x_kode_kegiatan_sub1"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<?php $tb_kegiatan_add->showPageHeader(); ?>
<?php
$tb_kegiatan_add->showMessage();
?>
<form name="ftb_kegiatanadd" id="ftb_kegiatanadd" class="<?php echo $tb_kegiatan_add->FormClassName ?>" action="<?php echo CurrentPageName() ?>" method="post">
<?php if ($tb_kegiatan_add->CheckToken) { ?>
<input type="hidden" name="<?php echo TOKEN_NAME ?>" value="<?php echo $tb_kegiatan_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tb_kegiatan">
<input type="hidden" name="action" id="action" value="insert">
<input type="hidden" name="modal" value="<?php echo (int)$tb_kegiatan_add->IsModal ?>">
...
<?php if ($tb_kegiatan->kode_kegiatan->Visible) { // kode_kegiatan ?>
	<div id="r_kode_kegiatan" class="form-group row">
		<label id="elh_tb_kegiatan_kode_kegiatan" for="x_kode_kegiatan" class="<?php echo $tb_kegiatan_add->LeftColumnClass ?>"><?php echo $tb_kegiatan->kode_kegiatan->caption() ?><?php echo ($tb_kegiatan->kode_kegiatan->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $tb_kegiatan_add->RightColumnClass ?>"><div<?php echo $tb_kegiatan->kode_kegiatan->cellAttributes() ?>>
<span id="el_tb_kegiatan_kode_kegiatan">
<input type="text" data-table="tb_kegiatan" data-field="x_kode_kegiatan" name="x_kode_kegiatan" id="x_kode_kegiatan" size="30" maxlength="40" placeholder="<?php echo HtmlEncode($tb_kegiatan->kode_kegiatan->getPlaceHolder()) ?>" value="<?php echo $tb_kegiatan->kode_kegiatan->EditValue ?>"<?php echo $tb_kegiatan->kode_kegiatan->editAttributes() ?>>
</span>
...
<?php if ($tb_kegiatan->nama_kegiatan->Visible) { // nama_kegiatan ?>
	<div id="r_nama_kegiatan" class="form-group row">
		<label id="elh_tb_kegiatan_nama_kegiatan" for="x_nama_kegiatan" class="<?php echo $tb_kegiatan_add->LeftColumnClass ?>"><?php echo $tb_kegiatan->nama_kegiatan->caption() ?><?php echo ($tb_kegiatan->nama_kegiatan->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $tb_kegiatan_add->RightColumnClass ?>"><div<?php echo $tb_kegiatan->nama_kegiatan->cellAttributes() ?>>
...
<?php if ($tb_kegiatan->kode_kegiatan_sub1->Visible) { // kode_kegiatan_sub1 ?>
	<div id="r_kode_kegiatan_sub1" class="form-group row">
		<label id="elh_tb_kegiatan_kode_kegiatan_sub1" class="<?php echo $tb_kegiatan_add->LeftColumnClass ?>"><?php echo $tb_kegiatan->kode_kegiatan_sub1->caption() ?><?php echo ($tb_kegiatan->kode_kegiatan_sub1->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $tb_kegiatan_add->RightColumnClass ?>"><div<?php echo $tb_kegiatan->kode_kegiatan_sub1->cellAttributes() ?>>
...
<script>
ftb_kegiatanadd.createAutoSuggest({"id":"x_kode_kegiatan_sub1","forceSelect":false});
</script>
...
<?php if (!$tb_kegiatan_add->IsModal) { ?>
<div class="form-group row"><!-- buttons .form-group -->
	<div class="<?php echo $tb_kegiatan_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit"><?php echo $Language->phrase("AddBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-href="<?php echo $tb_kegiatan_add->getReturnUrl() ?>"><?php echo $Language->phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<?php
$tb_kegiatan_add->showPageFooter();
if (DEBUG_ENABLED)
