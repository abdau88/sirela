...
class tb_user extends DbTable
{
	...
	public function __construct()
	{
		...
		// password
		$this->password = new DbField('tb_user', 'tb_user', 'x_password', 'password', '`password`', '`password`', 200, -1, FALSE, '`password`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'PASSWORD');
		$this->password->Required = TRUE; // Required field
		$this->password->Sortable = TRUE; // Allow sort
		$this->fields['password'] = &$this->password;

		// role
		$this->role = new DbField('tb_user', 'tb_user', 'x_role', 'role', '`role`', '`role`', 3, -1, FALSE, '`role`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'SELECT');
		...
		$this->role->Lookup = new Lookup('role', 'tb_user', FALSE, '', ["","","",""], [], [], [], [], [], [], '', '');
	...
	public function getRecordFilter($row = NULL)
	{
		...
		$val = is_array($row) ? (array_key_exists('username', $row) ? $row['username'] : NULL) : $this->username->CurrentValue;
		if ($val == NULL)
