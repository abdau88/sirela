...
ftb_useradd.validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		<?php } ?>
		<?php if ($tb_user_add->role->Required) { ?>
...
ftb_useradd.lists["x_role"] = <?php echo $tb_user_add->role->Lookup->toClientList() ?>;
ftb_useradd.lists["x_role"].options = <?php echo JsonEncode($tb_user_add->role->options(FALSE, TRUE)) ?>;
...
<input type="text" data-table="tb_user" data-field="x_username" name="x_username" id="x_username" size="30" maxlength="15" placeholder="<?php echo HtmlEncode($tb_user->username->getPlaceHolder()) ?>" value="<?php echo $tb_user->username->EditValue ?>"<?php echo $tb_user->username->editAttributes() ?>>
</span>
...
<input type="password" data-field="x_password" name="x_password" id="x_password" size="30" maxlength="10" placeholder="<?php echo HtmlEncode($tb_user->password->getPlaceHolder()) ?>"<?php echo $tb_user->password->editAttributes() ?>>
</span>
...
<?php if ($tb_user->role->Visible) { // role ?>
	<div id="r_role" class="form-group row">
		<label id="elh_tb_user_role" for="x_role" class="<?php echo $tb_user_add->LeftColumnClass ?>"><?php echo $tb_user->role->caption() ?><?php echo ($tb_user->role->Required) ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $tb_user_add->RightColumnClass ?>"><div<?php echo $tb_user->role->cellAttributes() ?>>
